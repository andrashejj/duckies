...
export function PersonLink({person}:{person:SocialPerson}){return <a className="social-person" href={`/members/people/${encodeURIComponent(person.id)}`}><span className="social-avatar" aria-hidden="true">{person.name.slice(0,1)}</span><span>{person.name}</span></a>}
