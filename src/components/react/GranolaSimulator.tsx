...
import { calculate, isStarterPack, newRecipe, recipeSchema, starterNames, starterPacks, starterRecipe, type PackId, type Recipe, type SavedPack, type Revision } from "../../lib/granola";
...
const taglines={"the-og":"The original mix","dawn-patrol":"Seeds for the early session","power-up":"Double the almonds"};
...
export default function GranolaSimulator() {
  ...
  const [active,setActive]=useState<PackId>("the-og");
    ...
    <div className="grid grid-cols-[minmax(0,1fr)_350px] items-start max-[1050px]:grid-cols-[minmax(0,1fr)_310px] max-[800px]:grid-cols-1 min-[1300px]:grid-cols-[minmax(0,1fr)_390px]">
      <fieldset className="m-0 min-w-0 border-0 p-0" disabled={loading||saving} key={active}>
        ...
        <section id="granola-mix" className={editorSection}><div className={sectionTitle}><span>01 / THE MIX</span><b>Recipe & ingredients</b></div>
          ...
          {/* Each row splits into two columns from 560px wide (2 × 272px + the gap), sized by its own grid. As a
              size container this list lost whole ingredient rows in Chromium once the saved recipes loaded. */}
          <div>{recipe.ingredients.map((item,index)=><div className="g-ingredient border-t border-line py-4" key={item.id}>
            ...
            <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,272px),1fr))] gap-4">
