  ...
  return <div ref={studio} className="granola-studio" aria-label="Granola recipe simulator" aria-busy={loading}>
    ...
    <div className="g-workbench">
      <fieldset className="g-editor" disabled={loading||saving} key={active}>
        ...
        <section id="granola-kitchen" className="g-editor-section"><div className="g-section-title"><span>02 / THE KITCHEN</span><b>Time & production</b></div><label className="g-field g-batch-policy"><span>Batch costing</span><select aria-label="Batch costing" value={recipe.batchCostMode??"proportional"} onChange={e=>change("batchCostMode",e.target.value as Recipe["batchCostMode"])}><option value="whole">Pay for each started batch</option><option value="proportional">Scale time and batch costs with volume</option></select></label><div className="g-fields-grid"><Field label="Packs per batch" value={recipe.batchPacks} min={1} onChange={v=>change("batchPacks",v)}/><Field label="Paid hours per batch" value={recipe.batchHours} suffix="h" step={0.25} onChange={v=>change("batchHours",v)}/><Field label="Loaded hourly cost" value={recipe.hourlyCost} suffix="Rs" onChange={v=>change("hourlyCost",v)}/><Field label="Ingredient waste allowance" value={recipe.wastePercent} suffix="%" max={100} onChange={v=>change("wastePercent",v)}/></div><p className="g-explain">Count everyone's paid time, including shopping, oven supervision and cleaning. The loaded rate includes employment costs. Waste is extra ingredient spending, separate from baking yield.</p></section>
        <section id="granola-other-costs" className="g-editor-section"><div className="g-section-title"><span>03 / THE REST</span><b>Packaging, overhead & paid time</b></div><p className="g-explain">Every row is editable. Choose rupees or paid hours, then when the cost applies. Paid hours use the loaded hourly rate above. Use “Each sold pack” for order handling or delivery that grows with sales.</p><div className="g-other-costs">{recipe.costs.map((item,index)=><div className="g-cost-entry" key={item.id}><div className="g-cost-row"><label className="g-field"><span>Cost name</span><input aria-label={`Cost ${index+1} name`} maxLength={80} value={item.name} onChange={e=>cost(index,"name",e.target.value)}/></label><Field label={`${item.name} amount`} displayLabel={item.calculation==="labour"?"Paid hours":"Amount"} value={item.amount} suffix={item.calculation==="labour"?"h":"Rs"} step={0.01} onChange={v=>cost(index,"amount",v)}/><label className="g-field"><span>Applies to</span><select aria-label={`${item.name} basis`} value={item.basis} onChange={e=>cost(index,"basis",e.target.value)}><option value="pack">Each made pack</option><option value="sold">Each sold pack</option><option value="batch">Each batch</option><option value="month">Each month</option></select></label><button type="button" className="g-remove" aria-label={`Remove ${item.name}`} onClick={()=>change("costs",recipe.costs.filter((_,i)=>i!==index))}>×</button></div><div className="g-cost-calculation"><label className="g-field"><span>Entered as</span><select aria-label={`${item.name} calculation`} value={item.calculation??"money"} onChange={e=>{const calculation=e.target.value as "money"|"labour";const amount=calculation==="labour"?(recipe.hourlyCost>0?item.amount/recipe.hourlyCost:0):item.amount*recipe.hourlyCost;change("costs",recipe.costs.map((row,i)=>i===index?{...row,calculation,amount}:row));}}><option value="money">Rupee amount</option><option value="labour">Paid hours × hourly rate</option></select></label><span>{result?`${money(result.monthlyRows[index].monthly,2)} / month`:"—"}</span></div></div>)}</div><button type="button" className="g-add" disabled={recipe.costs.length>=30} onClick={()=>change("costs",[...recipe.costs,{id:crypto.randomUUID(),name:"New cost",amount:0,basis:"month"}])}>+ Add cost</button></section>
        <section className="g-editor-section"><div className="g-section-title"><span>04 / THE BUSINESS</span><b>Price & monthly sales</b></div><div className="g-fields-grid"><Field label="Selling price" value={recipe.price} suffix="Rs" onChange={v=>change("price",v)}/><Field label="Packs produced monthly" value={recipe.monthlyPacks} onChange={v=>change("monthlyPacks",v)}/><Field label="Payment / selling fee" value={recipe.feePercent} suffix="%" max={100} step={0.5} onChange={v=>change("feePercent",v)}/><Field label="Retailer share" value={recipe.retailerPercent} suffix="%" max={100} step={0.5} onChange={v=>change("retailerPercent",v)}/></div><label className="g-slider"><span>Sell-through <strong>{recipe.sellThroughPercent}%</strong></span><input aria-label="Sell-through" type="range" min={0} max={100} value={recipe.sellThroughPercent} onChange={e=>change("sellThroughPercent",Number(e.target.value))}/><small>{result?.sold??0} sold / {recipe.monthlyPacks} made. Unsold stock is written off in this scenario.</small></label><p className="g-explain">Both selling fees and retailer share are percentages of the shelf price. Direct collection starts with zero retailer share. The batch policy determines whether partial batches pay full or proportional time and costs.</p></section>
      ...
      <aside ref={resultsPanel} id="granola-live-results" className="g-results" aria-label="Live business case">{result?<>
        ...
        <div className="g-month" aria-live="polite"><span>MONTHLY RESULT / CURRENT DRAFT</span><strong className={result.monthlyProfit<0?"negative":""}>{money(result.monthlyProfit)}</strong><p>{recipe.monthlyPacks} made · {result.sold} sold · {result.unsold} written off.<br/>Before tax, setup costs and club contributions.</p>
          <dl className="g-monthly-ledger" aria-label="Monthly cost breakdown">
            <div><dt>Sales revenue</dt><dd>{money(result.revenue,2)}</dd></div>
            <div><dt>Ingredients consumed</dt><dd>{money(result.monthlyIngredients,2)}</dd></div>
            <div><dt>Extra ingredient waste</dt><dd>{money(result.monthlyWaste,2)}</dd></div>
            <div><dt>Production labour</dt><dd>{money(result.monthlyLabour,2)}</dd></div>
            {result.monthlyRows.map(row=><div key={row.id}><dt>{row.name}<small>{row.calculation==="labour"?`${pretty(row.amount)} h × ${money(recipe.hourlyCost)} · `:""}{row.basis==="month"?"per month":row.basis==="batch"?"per batch":row.basis==="sold"?"per sold pack":"per made pack"}</small></dt><dd>{money(row.monthly,2)}</dd></div>)}
            <div><dt>Payment fees + retailer share</dt><dd>{money(result.monthlyFees,2)}</dd></div>
            <div className="g-ledger-total"><dt>Total monthly costs</dt><dd>{money(result.monthlyCost,2)}</dd></div>
          </dl><a className="g-edit-costs" href="#granola-other-costs">Edit overhead & other costs ↑</a>
