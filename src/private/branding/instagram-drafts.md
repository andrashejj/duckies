# Sunset Duckies — Instagram drafts (17 Sep 2026)

Ready-to-post captions. Files are under `public/media/instagram/` (stills) and `.local-data/instagram-video/` on Andras's Mac (videos, not committed). Every caption ends with the club hashtags; add a location tag "Tamarin Bay" (or "Le Morne Brabant" for the drone posts).

Hashtags: `#sunsetduckies #tamarin #tamarinbay #mauritius #kidssurf #groms #surfclub`

Consent: media yes/no is on each kid's signed waiver. Posts marked **check consent** show recognisable Duckies — confirm the families before posting. Posts marked **safe** show silhouettes, distant figures, adults or no people.

Web vs app: feed posts and Reels can go from instagram.com; Stories only from the phone.

---

## 1 · Fri 18 Sep · Save the date — feed, `photos/01-save-the-date.jpg` (4:5) · safe (one kid, back to camera)

Sunset Duckies Cup Vol. 02: Friday 16 October, Tamarin Bay. Heats for every level, crowns for the Duckie King and Queen, the grey goose heat for the parents. We are also cooking something for the day. Save the date; registration opens next week.

Alt text: A Duckie rides a small evening wave towards the beach, arms out, under a sunset sky.

## 2 · Mon 21 Sep · The gallery is live — carousel, 5 slides · check consent (slides 1 and 4)

Slides: `photos/feed-lineup.jpg` (4:3) · `stories/story-sunrise-rays-1.jpg` crop to 4:5 · `photos/01-save-the-date.jpg` · `photos/feed-beach-run.jpg` · `photos/feed-boards.jpg` (4:3) — Instagram forces one ratio per carousel: use 4:5 and let the two 4:3 ones crop, or post all at 1:1.

Six months of Mondays and Fridays, shot by the parents on the sand. The gallery is now on sunsetduckies.com — link in bio. Every photo there is a real session, a real Duckie and a real sunset. Got a better one? Drop it in the club WhatsApp.

## 3 · Wed 23 Sep · Reel — `feed-long-ride.mp4` (4:5, 10 s, silent) · check consent (one kid, face small)

Every session ends the same way: one more wave, all the way to the sand. Monday and Friday, 4–6pm, Tamarin Bay.

## 4 · Fri 25 Sep · Registration open — feed, `photos/02-registration-open.jpg` (4:5) · safe (silhouettes)

Registration for Cup Vol. 02 is open. Every Duckie surfs, whatever the level: divisions by age, a heat for everyone, prizes for more than the best wave. Sign up through the link in bio or the parents' WhatsApp. Judges, beach marshals and BBQ hands: we need you too.

## 5 · Mon 28 Sep · Reel — `feed-le-morne-arjon.mp4` (1:1, 25 s, silent) · safe (adult)

The grey goose heat is one heat for the parents at Cup Vol. 02. Same rules, same judges. This is what some of them have been doing on Sunday mornings while the Duckies build sandcastles. Le Morne, 12 July, early.

Location: Le Morne Brabant.

## 6 · Wed 30 Sep · Reel — `feed-three-on-a-wave.mp4` (4:5, 6.5 s, silent) · check consent (three kids + coach)

Three Duckies, one wave, one coach doing the maths. Heats of four at the Cup, so this is practice.

## 7 · Fri 2 Oct · Teaser 1 — real photo of a tray out of the oven (to be shot) · safe

Something is in the oven in Tamarin. First test batch this week, tasted by the founding families at Friday training. More at the Cup.

## 8 · Mon 5 Oct · Reel — `feed-sunrise-lineup.mp4` (4:5, 8.5 s, ambient audio) · safe (silhouettes)

Sunrise Duckies, for one morning. The lineup at Tamarin on 12 July, shot from the sand by Holly. Same bay, other end of the day.

## 9 · Wed 7 Oct · Feed — `photos/feed-boards.jpg` (4:3) · safe (feet only)

Boards by the Duckies. Four soft-tops, four artists, zero brand guidelines. The rashies at the Cup are colour-coded; the boards clearly are not.

## 10 · Fri 9 Oct · The maker — real photo of Estelle in the kitchen (to be shot) · safe

Meet the baker. Estelle is in Tamarin for three weeks, in the kitchen and on the beach, making something for surf mornings. Oats, nuts and honey, made by hand a few streets from the bay. What it is and what it's called: at the Cup.

## 11 · Mon 12 Oct · Countdown — feed, `photos/05-countdown.jpg` (4:5) · safe (silhouette)

Four days. Heats start at [time] on Friday; the schedule is on sunsetduckies.com. Bring sunscreen, water and your cheering voice. The first batch gets revealed at the stall before the crowns.

## 12 · Wed 14 Oct · Reel — `feed-le-morne-beach.mp4` (1:1, 31 s, silent) · check consent (two Duckies from far above)

Two days. The beach the drone found while the parents surfed: Le Morne, from above, with two Duckies and a lot of sand. Friday we are on our own beach.

Location: Le Morne Brabant.

## 13 · Thu 15 Oct · Reel — `feed-bonfire.mp4` (4:5, 20 s, ambient audio) · safe (no people)

Tomorrow. Heats from [time], crowns before sunset, and the evening ends the way club evenings end.

## 14 · Fri 16 Oct · The reveal — real photo of the stall (to be shot), plus Stories from the beach

Here it is: [name], the first Sunset Duckies granola, baked in Tamarin. Rs [price] a bag at the stall today; pre-orders for the next batch through the link in bio.

## 15 · Sat 17 – Sun 18 Oct · Results — real photo of the crowns · check consent

Cup Vol. 02, done. [Duckie King and Queen, Duckling King and Queen.] Thank you to the judges, the marshals, the BBQ crew and every parent on the sand. [X] bags left / sold out: order the next batch through the link in bio.

---

## Stories (from the phone, any time) · all safe

`stories/story-sunrise-rays-1.jpg`, `story-sunrise-rays-2.jpg`, `story-sunrise-red-sun.jpg`, `story-sunrise-sky.jpg`, `story-bonfire.jpg` — stills.
`story-evening-ride.mp4`, `story-pastel-sunset-surf.mp4` — safe. `story-kids-tandem.mp4`, `story-beach-games.mp4`, `story-board-carry.mp4` — check consent.

Sticker ideas: "Cup Vol. 02 · Fri 16 Oct" countdown sticker on any of the sunrise stills; link sticker to sunsetduckies.com/gallery on the first Story after post 2.

## Bio

Kids' surf club in Tamarin Bay 🌅 · Mon + Fri, 4–6pm · Cup Vol. 02: Friday 16 October
Website: https://sunsetduckies.com
