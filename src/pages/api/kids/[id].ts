...
const update: APIRoute = async ({ request, params }) => {
  try {
    ...
    if (request.method === "DELETE") {
      const result = await getDatabase().query("UPDATE club_kid SET archived_at=now() WHERE id = $1 AND archived_at IS NULL RETURNING id", [params.id]);
    ...
    const result = await getDatabase().query("UPDATE club_kid SET name = $1 WHERE id = $2 AND archived_at IS NULL RETURNING id, name", [name, params.id]);
