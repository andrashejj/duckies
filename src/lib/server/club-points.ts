...
import { canCallRoll, clubToday, emptyPoints, type KidPoints, type PointActivity, type PointRules, type PointSplit, type TrainingAccess, type TrainingBoard } from "../club-points";
...
export async function trainingBoard(date: string, email: string, access: TrainingAccess): Promise<TrainingBoard> {
  ...
  const [kids, sessions, session, rules] = await Promise.all([
    db.query(`SELECT k.id,k.name,ph.updated_at AS "photoVersion",EXISTS(SELECT 1 FROM club_current_guardian g WHERE g.kid_id=k.id AND g.email=$2) AS mine,
      ...
      FROM club_kid k LEFT JOIN club_kid_photo ph ON ph.kid_id=k.id LEFT JOIN club_attendance a ON a.kid_id=k.id AND a.date=$1::date
      LEFT JOIN (SELECT kid_id,count(*)::int AS trainings FROM club_attendance WHERE present GROUP BY kid_id) t ON t.kid_id=k.id
      WHERE k.archived_at IS NULL ORDER BY lower(k.name),k.id`, [date, email.trim().toLowerCase()]),
  ...
  return { date, today: clubToday(), access, sessionPoints: session.rows[0]?.points ?? rules.training, sessions: sessions.rows,
...
/** Organisers and coaches call the roll; each duckie there earns the training's points. */
export async function markAttendance(date: string, kidIds: string[], present: boolean, actor: string) {
  ...
  try {
    ...
    const saved = await db.query(`INSERT INTO club_attendance(date,kid_id,present,updated_by) SELECT $1::date,id,$3,$4 FROM unnest($2::uuid[]) AS id
      ON CONFLICT(date,kid_id) DO UPDATE SET present=EXCLUDED.present,updated_by=EXCLUDED.updated_by,updated_at=clock_timestamp()
      WHERE club_attendance.present IS DISTINCT FROM EXCLUDED.present RETURNING kid_id`, [date,ids,present,actor]);
    if (saved.rowCount) await db.query("INSERT INTO club_attendance_event(date,kid_id,present,recorded_by) SELECT $1::date,id,$3,$4 FROM unnest($2::uuid[]) AS id", [date,saved.rows.map(row => row.kid_id),present,actor]);
