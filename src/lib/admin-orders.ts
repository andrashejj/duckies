...
import type { OrderStatusVariant } from "./email/send";
...
export async function transitionOrder(opts: {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    if (!STATUS_TRANSITIONS[existing.status].includes(to)) {
      throw new Error(`Can't move from ${STATUS_LABEL[existing.status]} to ${STATUS_LABEL[to]}.`);
    }
    await tx.order.update({ where: { id: orderId }, data: { status: to } });
    if (to === OrderStatus.CANCELLED) {
      // Restore only inventory that this reservation actually deducted.
      for (const item of [...existing.items].sort((a, b) => a.productId.localeCompare(b.productId))) {
        if (item.stockReserved) await tx.product.updateMany({
          where: { id: item.productId, stock: { not: null } }, data: { stock: { increment: item.quantity } },
        });
      }
    }
    await tx.orderEvent.create({ data: { orderId, type: statusToEventType[to], actorId,
      message: adminNote ? `Moved to ${STATUS_LABEL[to]} — ${adminNote}` : `Moved to ${STATUS_LABEL[to]}.` } });
    return tx.order.findUniqueOrThrow({ where: { id: orderId }, include: { items: true, events: { orderBy: { createdAt: "desc" } } } });
  });

  const emailVariant = statusToEmailVariant[to];
  if (sendEmail && emailVariant) {
    const result = await sendOrderStatusEmail(updated, emailVariant);
    if (result.ok) {
...
export async function markOrderPaid(opts: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (!existing) throw new Error("Reservation not found.");
    if (existing.paidAt) throw new Error("Reservation is already marked paid.");
    if (existing.status === OrderStatus.CANCELLED) throw new Error("A cancelled reservation cannot be marked paid.");
    await tx.order.update({ where: { id: orderId }, data: { paidAt: new Date() } });
    await tx.orderEvent.create({ data: { orderId, type: OrderEventType.PAID, actorId,
