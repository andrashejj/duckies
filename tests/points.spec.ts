...
const check = (request: APIRequestContext, kidId = alice, present = true, date = today) => request.patch("/api/training", post({ date, kidIds: [kidId], present }));
...
test("organisers call the roll, members only read it and only Andras sets point values; retries and undo preserve one award and an audit trail", async ({ request }) => {
  ...
  expect((await ownerRequest.patch("/api/training",{headers:{origin:"https://evil.example"},data:{date:today,kidIds:[alice],present:true}})).status()).toBe(403);
  ...
  expect((await ownerRequest.patch("/api/training",post({date:today,kidIds:[],present:true}))).status()).toBe(400);
  ...
  expect((await ownerRequest.patch("/api/training",post({date:today,kidIds:[alice,randomUUID()],present:false}))).status()).toBe(404);
