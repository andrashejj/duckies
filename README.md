...
`/members` is the shared feed: text and photo posts, likes, comments, member profiles, and private post links. `/members/lineup` retains the roster. `/members/me` collects a member's posts, their uploaded photos and photos tagged to their own duckies. Family records, profile settings, orders, the photo library and upload page share the member navigation and light/dark theme. Social profiles expose display names only; existing private registration and adult profile photos are not republished as social avatars. Legacy gallery uploads stay in the library without newly attributing them in the feed.
