...
Registration comes before payment. Families register themselves at `/register`: one form for the club, the Cup alone, or both. Opening it mints a draft link (`POST /api/registration/draft`) that belongs to no kid yet; photos stage against it, and nothing reaches the roster until the family signs. Signing matches each child to an unsigned kid already on the roster (same name or first name, and a guardian's phone number) or creates them as *pending*, and from then on the draft is an ordinary link for corrections and the download. A kid who already has a signed registration is not re-signed from the public form: their guardians sign in and get a fresh link from their family page. An organiser can also add a kid and **Generate registration link** from the roster at any time; choose **Copy link** or **Send via WhatsApp**. The kid becomes a *member* once the registration is signed **and** the current semester is marked paid — and only `andras@hejj.xyz` can record payments (cash or transfer, confirmed by hand). Every current legal guardian can sign in to their own family. Full club membership follows the kid: once the registration is signed and the current semester is paid, every current legal guardian is a club member (`grantGuardianMembership` runs on signing, on payment and when a guardian is added; removing someone from their last shared duckie ends it). Signing alone never grants it, since `/register` is public. Each guardian also has an **Approve membership** / **Revoke membership** button in the kid's overview (`POST`/`DELETE /api/members`) for the hand-operated cases; a revoked guardian is granted again by the next registration, payment or guardian change for their family. A member's co-guardians can use the shop without receiving the full roster or gallery. Public drafts are rate limited to 8 per 10 minutes per address. The latter opens a message for you to send; the app does not send WhatsApp messages itself. Links use 256-bit random tokens, with only their hashes stored in the database. Tokens travel in the URL fragment, not query strings or page requests, so link previews and ordinary server access logs do not receive them. The invitation expires after 14 days. Generating another link revokes prior links for that child and semester; the organiser can also revoke a link directly. Do not log request Authorization headers or signing bodies in external infrastructure.
...
**Cup registration** reuses the same form. The Cup is its own term (`cup-vol-2`, seeded by migration with the Rs 1,000 entry fee; constants in `src/lib/registration/cup.ts`). On `/sunset-duckies-cup-vol-2` a member family signs in (`/login?next=…`) with the email on their club registration; `GET /api/cup/kids` lists only the kids whose latest signed registration names that email, and `POST /api/cup/kids/:id` puts one on the list — no form, no waiver. The entry is free while the kid is a member (signed + current semester paid); otherwise the roster shows the Rs 1,000 fee. A new family goes to `/register?plan=cup` (Cup only) or `/register?plan=both` (club + Cup): signing puts every child on the form on the Cup list, and the entry fee is recorded like a semester payment under the Cup term. A signed-in family adding a sibling goes to `/register?plan=both` with their details carried over. Cup forms hide the membership-only parts of the form and do not require a training rhythm. The roster flags entries as **Cup 02** and offers a "Coming to the Cup" filter.
...
**Admin → Training & points** (`/admin/training`) opens today's roll in Mauritius time. Choose another date to record or correct a past training, search by duckie or parent, and check each child as they arrive. Each change saves immediately. Only the verified organiser `andras@hejj.xyz` can change attendance or point values; other organisers can view them. Future attendance is rejected on the server. A session picker on the same page switches between **Sunset Duckies** (the club's evening training) and **Sunrise Duckies** (an early session); a child earns points once per date and session, so the two can both be rolled on the same day without conflict. Unchecking reverses points, and all actual changes retain an actor-stamped audit event. No attendance is inferred or backfilled.

Initial values are **10 per Sunset Duckies training**, **5 per Sunrise Duckies training**, **4 per granola bag reserved for a selected child**, and **20 per Cup edition with a scored run**. Values can be changed under **Point values**, including 0 to disable a category for new activities. A training keeps the value recorded when its roll was first saved. A granola award lands as soon as the reservation is placed — a family doesn't wait for an organiser to mark it paid — using the rule in force when the order was placed; Cup participation uses the first scored run. Changing rules does not rewrite existing awards. Cancelled orders earn no points, whether or not they were ever paid. Multiple judges, runs and heats still earn only one participation award per child per edition. These totals are separate from competition star ratings and standings.
...
The admin page refreshes totals every 20 seconds. Guardians can see each child's total, category breakdown and activity history on **My family**, restricted to their own linked children. `club_point_activity` derives awards from attendance, granola orders and Cup runs so duplicate requests cannot create duplicate points. Apply both migrations before deploying this app version.

Migration `20260926120000_club_sunrise_duckies` adds Sunrise Duckies alongside Sunset Duckies: `club_training`, `club_attendance` and `club_attendance_event` gain a `session_type` (`sunset` or `sunrise`) as part of their key, and `club_point_rule` gains a `sunrise` rate (5 by default). Existing rows are backfilled to `sunset`, so nothing already recorded changes value or meaning.

Migration `20260926130000_club_granola_on_order` moves granola's award point from "marked paid" to "reservation placed" — `club_point_activity`'s granola branch now keys off `Order.createdAt` instead of `Order.paidAt`, and no longer requires `paidAt IS NOT NULL` (only `status<>'CANCELLED'`). It also lowers the granola rate from 5 to 4, carrying the current training, sunrise and Cup rates forward unchanged in a new `club_point_rule` row. Because the view recomputes live, any order still pending when this ships starts earning points immediately, and an already-paid order's rate is now looked up by when it was placed rather than when it was paid (only matters for an order that straddled a rate change). Verify a fresh reservation earns points before it's marked paid, and that cancelling it removes them.
