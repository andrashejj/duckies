...
**Cup registration** reuses the same form. The Cup is its own term (`cup-vol-2`, seeded by migration with the Rs 1,000 entry fee; constants in `src/lib/registration/cup.ts`), and the public form on `/sunset-duckies-cup-vol-2` posts to `/api/cup/register`. A family that says it is a club member is matched by the kid's name plus a guardian, emergency or contact number from the signed club registration — no form, no waiver, just a `club_cup_entry` row. A new family gets a kid row, a cup entry and a self-issued private link to `/register` for the cup term (no payment gate; the entry fee is recorded like a semester payment under the Cup term). Cup links hide the membership-only parts of the form and do not require a training rhythm. The roster flags entries as **Cup 02** and offers a "Coming to the Cup" filter.
